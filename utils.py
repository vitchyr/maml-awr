...
class ReplayBuffer(object):
    def __init__(self, trajectory_length: int, state_dim: int, action_dim: int, max_trajectories: int = 100000,
                 discount_factor: float = 0.99):
        self._trajectories = np.empty((max_trajectories, trajectory_length, state_dim + action_dim + state_dim + 1 + 1))
    ...
    def add_trajectory(self, trajectory: List[Experience]):
        ...
        for idx, experience in enumerate(trajectory[::-1]):
            slice_idx = 0
            self._trajectories[self._stored_trajectories, -(idx + 1), slice_idx:slice_idx + self._state_dim] = experience.state
            slice_idx += self._state_dim

            self._trajectories[self._stored_trajectories, -(idx + 1), slice_idx:slice_idx + self._action_dim] = experience.action
            slice_idx += self._action_dim

            self._trajectories[self._stored_trajectories, -(idx + 1), slice_idx:slice_idx + self._state_dim] = experience.next_state
            slice_idx += self._state_dim

            self._trajectories[self._stored_trajectories, -(idx + 1), slice_idx:slice_idx + 1] = experience.reward
            slice_idx += 1

            mc_reward = experience.reward + self._discount_factor * mc_reward

            self._trajectories[self._stored_trajectories, -(idx + 1), slice_idx:slice_idx + 1] = mc_reward
